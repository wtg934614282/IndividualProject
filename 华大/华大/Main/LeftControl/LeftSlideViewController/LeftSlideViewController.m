...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    else
    {
        ...
        {
            [self closeLeftViewWithAnimated:YES];
            _scalef = 0;
        }
        else if (self.mainVC.view.x > (kScreenWidth - kMainPageDistance))
        {
            [self openLeftViewWithAnimated:YES];
            _scalef = 0;
        }
    }
...
/**
 @brief 关闭左视图
 */
- (void)closeLeftViewWithAnimated:(BOOL)animated
{
    if (animated) {
        [UIView beginAnimations:nil context:nil];
...
/**
 @brief 打开左视图
 */
- (void)openLeftViewWithAnimated:(BOOL)animated
{
